...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/playwright-1.40+-orange.svg" alt="Playwright Version">
...
- 🤖 **Browser Agent智能爬虫**: 基于Playwright的AI驱动浏览器，自动识别网页元素，无需硬编码CSS选择器
...
│   │   │   ├── weibo_playwright.py # 微博爬虫（含评论抓取）
│   │   │   ├── zhihu_playwright.py  # 知乎爬虫（含评论抓取）
│   │   │   ├── tieba_playwright.py  # 贴吧爬虫（含评论抓取）
│   │   │   └── hupu_playwright.py   # 虎扑爬虫（含评论抓取）
...
### 0. Browser Agent 智能爬虫（论文创新亮点）

基于 Playwright 的 AI 驱动浏览器 Agent，**不依赖硬编码 CSS 选择器**，通过页面结构分析自动识别内容区域：
...
print(f"获取 {len(df)} 条微博内容")
```

**核心创新**：
- 自动识别网页元素，无需维护 CSS 选择器
- 智能滚动 + 内容去重
- 评论深度采集（进入详情页抓取评论）
- Cookie 智能复用
...
| 爬虫框架 | Playwright | 1.40+ | 多平台数据爬取（当前方案）|
| 爬虫框架 | Selenium | 4.20+ | 多平台数据爬取（原有方案，已弃用）|
...
- [x] Docker容器化部署 ✨ 新增
- [x] Playwright爬虫框架 ✨ 新增
- [x] Browser Agent智能爬虫 ✨ 新增
- [x] 四大平台评论抓取（微博/知乎/虎扑/贴吧）✨ 新增
- [ ] 知识图谱前端可视化集成
- [ ] 实时流式数据处理
