    ...
    def __init__(self, headless=False):
        ...
        for attempt in range(max_retries):
            try:
                ...
                print(f"✅ 微博爬虫 driver 创建成功 (尝试 {attempt+1}/{max_retries})")
                break
            except Exception as e:
                print(f"❌ 微博爬虫 driver 创建失败 (尝试 {attempt+1}/{max_retries}): {e}")
                ...
                try:
                    ...
                    if comment_content and comment_content not in seen_texts:
                        ...
                        filter_words = ['按热度', '按时间', '加载更多', '查看更多', '回复', '赞', '转发', '收藏', '评论', '来自', '热门', '最新', '全部', '消息', '发布于', '播放视频',
                                      '粉丝', '关注', '视频累计播放量', '本地资讯博主', 'IP属地', '展开', '常用功能', '为TA助威', '关注推荐']
                        should_skip = False
                        for word in filter_words:
                            if word in comment_content and len(comment_content) < 50:
                                should_skip = True
                                break
                        
                        # 过滤掉纯数字和时间格式的内容
                        if re.match(r'^\d+$', comment_content.strip()) or re.match(r'^\d{2},\d{2}$', comment_content.strip()):
                            should_skip = True
                        ...
                        if not should_skip:
                            ...
                            if comment_content and len(comment_content) > 0:
                                # 确保用户名称不是默认值或无意义的内容
                                if user_name == "未知用户" or len(user_name) < 2:
                                    # 尝试从评论内容中提取用户名
                                    if ':' in comment_content or '：' in comment_content:
                                        sep_idx = comment_content.find(':') if ':' in comment_content else comment_content.find('：')
                                        if sep_idx != -1:
                                            possible_user = comment_content[:sep_idx].strip()
                                            if possible_user and len(possible_user) > 1 and len(possible_user) < 50:
                                                user_name = possible_user
                                                comment_content = comment_content[sep_idx+1:].strip()
            ...
            while post_counter < target_count and loop_count < max_loops:
                ...
                for idx, post in enumerate(posts):
                    ...
                    try:
                            ...
                            for link_idx, link in enumerate(all_links):
                                try:
                                    ...
                                    if href and 'weibo.com/' in href:
                                        # 直接检查是否是微博详情页链接
                                        if 'weibo.com/5200478600/' in href:
                                            post_url = href
                        ...
                        if not post_url:
                            try:
                                links = post.find_elements(By.CSS_SELECTOR, 'a[href*="/detail/"], a[href*="/status/"]')
                                for link in links:
                                    try:
                                        href = link.get_attribute('href')
                                        if href:
                                            # 过滤掉用户主页链接
                                            if '/u/' in href or '/100808' in href or 'profile' in href:
                                                print("    跳过用户主页链接")
                                                continue
                                            post_url = href
                            ...
                            for click_round in range(5):
                                ...
                                for selector in more_selectors:
                                    try:
                                        ...
                                        time.sleep(1)
                                        elem.click()
                                        print("点击了查看更多评论 (第%d轮)" % (click_round + 1))
                                        time.sleep(3)
                                        clicked_any = True
                                        for i in range(30):
                                            self.driver.execute_script("window.scrollTo(0, document.body.scrollHeight);")
                            ...
                            if len(comments_data) == 0:
                                ...
                                try:
                                    ...
                                    while i < len(lines):
                                        ...
                                        should_skip = False
                                        for word in filter_words:
                                            if word in line and len(line) < 50:
                                                should_skip = True
                                                break
                                        
                                        # 过滤掉纯数字和时间格式的内容
                                        if re.match(r'^\d+$', line.strip()) or re.match(r'^\d{2},\d{2}$', line.strip()):
                                            should_skip = True
                                        ...
                                        if line and len(line) > 5 and line not in seen_texts:
                                            user_name = "未知用户"
                                            ...
                                            # 确保评论内容有一定长度
                                            if comment_content and len(comment_content) > 4:
                                                comments_data.append({"user": user_name, "content": comment_content})
                                                seen_texts.add(line)
