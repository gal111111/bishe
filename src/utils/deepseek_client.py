...
class DeepSeekClient:
    ...
    def __init__(self, api_key: Optional[str] = None, base_url: Optional[str] = None, model: Optional[str] = None):
        ...
        if self.api_key:
            ...
            print("[WARN] 未设置DeepSeek API密钥，DeepSeek能力将不可用")
    ...
    def chat_completion(self, messages: List[Dict[str, str]], model: Optional[str] = None, temperature: float = 0.3, max_tokens: int = 1000) -> Dict[str, Any]:
        ...
        if not self.api_key:
            raise RuntimeError("DeepSeek API密钥未设置，无法调用API")
        ...
        try:
            ...
            raise RuntimeError("DeepSeek ChatCompletion 调用失败") from e
    
    def embedding(self, input: str, model: str = "deepseek-embedding-v1.0") -> Dict[str, Any]:
        """调用Embedding API"""
        if not self.api_key:
            raise RuntimeError("DeepSeek API密钥未设置，无法调用Embedding API")
        ...
        try:
            ...
            raise RuntimeError("DeepSeek Embedding 调用失败") from e
