...
class LatestDataProcessor:
    def __init__(self):
        ...
        self.latest_data_dir = os.path.join(self.project_root, "data", "latest")
        self.merged_data_path = os.path.join(self.project_root, "data", "latest", "merged_all_platform.csv")
        self.cleaned_data_path = os.path.join(self.project_root, "data", "latest", "cleaned_all_data.csv")
        self.analysis_result_path = os.path.join(self.project_root, "data", "latest", "analysis_result.json")
    
    def load_platform_data(self, platform: str, keyword: str = "上海迪士尼") -&gt; pd.DataFrame:
        """加载单个平台的CSV数据"""
        try:
            # 查找平台的CSV文件
            for file_name in os.listdir(self.latest_data_dir):
                if file_name.startswith(f"{platform}_raw_{keyword}") and file_name.endswith(".csv"):
                    file_path = os.path.join(self.latest_data_dir, file_name)
    ...
    def analyze_data(self, df: pd.DataFrame) -&gt; Dict[str, Any]:
        ...
        # 保存分析后的评论数据
        analyzed_comments_path = os.path.join(self.project_root, "data", "latest", "analyzed_comments.csv")
        if "polarity_label" in analyzed_df.columns:
