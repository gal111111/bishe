...
# 查找所有上海迪士尼的文件
disney_files = [f for f in os.listdir(RAW_DIR) if "上海迪士尼" in f and f.endswith(".csv")]
if disney_files:
    # 提取日期并找到最新的
    dates = []
    for f in disney_files:
        match = re.search(r'上海迪士尼_(\d{8})\.csv', f)
        if match:
            dates.append(match.group(1))
    if dates:
        date_str = sorted(dates)[-1]  # 最新的日期
        print(f"自动选择最新日期: {date_str}")
    else:
        date_str = datetime.now().strftime("%Y%m%d")
        print(f"未找到日期，使用今天的日期: {date_str}")
else:
    date_str = datetime.now().strftime("%Y%m%d")
    print(f"未找到上海迪士尼文件，使用今天的日期: {date_str}")
