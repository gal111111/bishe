...
def show_dashboard(df):
        ...
        <div class="main-header">
            <div style="display: flex; align-items: center; gap: 24px; position: relative; z-index: 1;">
                ...
                <div>
                    ...
                    <p style="margin: 8px 0 0 0; color: rgba(148, 163, 184, 0.9); font-size: 15px;">实时监控城市公共设施服务质量与市民满意度</p>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with c4:
        ...
        <div class="stat-card">
            <h3>正面评价</h3>
            <div class="value" style="color: #10b981;">{pos_rate:.1f}%</div>
    ...
    with c6:
        ...
        <div class="stat-card">
            <h3>负面评价</h3>
            <div class="value" style="color: #ef4444;">{neg_rate:.1f}%</div>
    ...
    with tab2:
        ...
        if selected_filename:
            ...
            try:
                ...
                is_valid, validation_msg = validate_csv_data(df_raw)
                if not is_valid:
                    st.error(f"数据验证失败: {validation_msg}")
                    return
                ...
                with col2:
                    with open(selected_filepath, "rb") as f:
                        safe_filename = sanitize_filename(selected_filename)
                        st.download_button("💾 下载此原始数据", f, safe_filename, type="secondary")
...
def page_report():
    ...
    if not md_files:
        st.markdown("""
        <div style="text-align: center; padding: 60px 20px; background: rgba(30, 41, 59, 0.6); border-radius: 20px; border: 1px solid rgba(59, 130, 246, 0.2);">
    ...
    with tab1:
        ...
        with col_right:
            ...
            <div class="card">
                ...
                <p style="color: #10b981; margin: 8px 0;">✅ 执行摘要</p>
                <p style="color: #10b981; margin: 8px 0;">✅ 数据采集概况</p>
                <p style="color: #10b981; margin: 8px 0;">✅ 情感分析深度解析</p>
                <p style="color: #10b981; margin: 8px 0;">✅ 方面级满意度分析</p>
                <p style="color: #10b981; margin: 8px 0;">✅ 改进建议与对策</p>
            </div>
            """, unsafe_allow_html=True)
    ...
    with tab3:
        ...
        with col_dl1:
            st.markdown("""
            <div style="background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%); border-radius: 16px; padding: 24px; text-align: center; margin-bottom: 16px;">
                <div style="font-size: 48px; margin-bottom: 8px;">📄</div>
                <h3 style="margin: 0; color: #ffffff;">Markdown 报告</h3>
                <p style="color: rgba(255,255,255,0.8); font-size: 13px;">学术研究报告</p>
            </div>
            """, unsafe_allow_html=True)
            safe_report_name = sanitize_filename(f"舆情分析报告_{time.strftime('%Y%m%d')}.md")
            st.download_button(
                label="⬇️ 下载 Markdown",
                data=content,
        ...
        with col_dl2:
            st.markdown("""
            <div style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); border-radius: 16px; padding: 24px; text-align: center; margin-bottom: 16px;">
                <div style="font-size: 48px; margin-bottom: 8px;">📊</div>
            ...
            if os.path.exists(res_path):
                with open(res_path, "rb") as f:
                    csv_data = f.read()
                safe_csv_name = sanitize_filename(f"分析数据_{time.strftime('%Y%m%d')}.csv")
                st.download_button(
                    label="⬇️ 下载 CSV",
                    data=csv_data,
                    file_name=safe_csv_name,
                    mime="text/csv",
                    type="primary",
                    use_container_width=True
                )
            ...
            for i, md_file in enumerate(md_files[:5]):
                ...
                with col_h3:
                    with open(md_file, "r", encoding="utf-8") as f:
                        hist_content = f.read()
                    safe_hist_name = sanitize_filename(os.path.basename(md_file))
                    st.download_button(
                        label="下载",
                        data=hist_content,
                        file_name=safe_hist_name,
                        mime="text/markdown",
                        key=f"hist_{i}"
                    )
...
def main():
    ...
    with st.sidebar:
        ...
        <div class="sidebar-logo">
            ...
            <h2 style="margin: 0; font-size: 22px; font-weight: 800;">城市舆情态势感知中心</h2>
            <p style="margin: 5px 0 0 0; color: rgba(148, 163, 184, 0.9); font-size: 12px;">城市公共设施舆情分析系统</p>
        </div>
        """, unsafe_allow_html=True)
